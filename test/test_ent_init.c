...
static int test_ent_close_clears_runtime_handles(void)
{
    ...
    if(expect_true(ENT_Close() == 0, "ENT_Close should succeed for an initialized context") != 0)
    {
        return 1;
    }
...
static int test_ent_init_builds_paths_without_trailing_separator(void)
{
    ...
    if(expect_true(ENT_Close() == 0, "ENT_Close should succeed after a successful ENT_Init") != 0)
    {
        return 1;
    }
...
static int test_ent_init_builds_paths_with_trailing_separator(void)
{
    ...
    return expect_true(ENT_Close() == 0, "ENT_Close should succeed after trailing-separator initialization");
}
